...
type Result = { status: "completed"; assessment: Assessment; botSections: string[]; checkedAt: string };
...
export default function CsAssistance() {
  ...
  async function submit(e: FormEvent) {
    e.preventDefault();
    if (!validId || !canCheck || loading) return;
    controller.current?.abort();
    const run = new AbortController(); controller.current = run;
    setLoading(true); setResult(null); setError(""); setCopied(false); setTab("response");
    setCheckedInput({ idfv: normalized, asOf });
    try {
      const started = await post("/api/cs-assistance", { idfv: normalized, asOf }, run.signal);
    ...
    <div className="grid items-start gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="rounded-2xl border border-line bg-surface-card p-5">
        ...
        <form onSubmit={submit} className="mt-6 space-y-5">
          ...
          <button type="submit" disabled={!validId || !canCheck || loading || (!!asOf && asOf > today)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-cobalt px-4 py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40">{loading ? <Loader2 size={16} className="animate-spin" /> : <Search size={16} />}{loading ? "Checking history…" : "Check player"}</button>
          {loading ? <button type="button" onClick={cancel} className="flex w-full items-center justify-center gap-2 text-xs text-text-subtle"><X size={14} /> Stop waiting</button> : <button type="button" disabled={!canCheck} onClick={() => { setIdfv("38645146fe0ea5644f7853ee3d88f77e"); setAsOf("2026-09-08"); }} className="flex w-full items-center justify-center gap-2 text-xs font-medium text-cobalt disabled:opacity-40">Use the VIP Pass example <ArrowRight size={13} /></button>}
        </form>
