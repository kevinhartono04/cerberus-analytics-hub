...
const inputClass = "focus-ring h-[42px] w-full rounded-[9px] border border-line/70 bg-[#0a111e] px-3 text-sm font-semibold text-slate-300 placeholder:font-normal placeholder:text-slate-500";
  ...
  return (
    <label className="block">
      ...
      <div className="relative" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setIsOpen(false); }}>
        ...
        {isOpen ? <div className="absolute left-0 top-full z-50 mt-2 max-h-72 w-full overflow-y-auto rounded-[9px] border border-line/70 bg-[#0d1424] p-1 shadow-soft">
          {options.map((option) => <button key={option.value} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { onChange(option.value); setIsOpen(false); }} className={`focus-ring block w-full rounded-[7px] px-3 py-2 text-left text-sm font-semibold transition-colors hover:bg-[#17223a] ${option.value === value ? "bg-emerald/10 text-emerald" : "text-slate-400"}`}>{option.label}</button>)}
        </div> : null}
      </div>
    </label>
  );
  ...
  return (
    <div className="block">
      ...
      <div className="relative" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setIsOpen(false); }}>
        ...
        {isOpen ? <div className="absolute left-0 top-full z-50 mt-2 max-h-80 w-full overflow-y-auto rounded-[9px] border border-line/70 bg-[#0d1424] p-1 shadow-soft">
          <div className="sticky top-0 bg-[#0d1424] p-1">
            <input autoFocus value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addTypedVersion(); } }} placeholder="Search or type a version" className="focus-ring h-9 w-full rounded-[7px] border border-line/70 bg-[#0a111e] px-2 text-sm text-slate-200 placeholder:text-slate-500" />
          </div>
          <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => onChange([])} className={`focus-ring block w-full rounded-[7px] px-3 py-2 text-left text-sm font-semibold hover:bg-[#17223a] ${!values.length ? "bg-emerald/10 text-emerald" : "text-slate-400"}`}>All versions</button>
  ...
  return (
    <label className="block">
      ...
      <div className="relative" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setIsOpen(false); }}>
        ...
        {value ? <button type="button" onClick={() => { onChange(""); setIsOpen(true); }} className="focus-ring absolute right-9 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-[6px] text-slate-500 hover:bg-[#17223a] hover:text-slate-200" aria-label="Clear app version"><XCircle className="h-4 w-4" /></button> : null}
        <button type="button" onClick={() => setIsOpen((open) => !open)} className="focus-ring absolute right-2 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-[6px] text-slate-500 hover:bg-[#17223a] hover:text-slate-200" aria-label="Toggle app version suggestions" aria-expanded={isOpen}><ChevronDown className={`h-4 w-4 transition-transform ${isOpen ? "rotate-180" : ""}`} /></button>
        {isOpen ? <div id="level-funnel-version-options" role="listbox" className="absolute left-0 top-full z-50 mt-2 max-h-72 w-full overflow-y-auto rounded-[9px] border border-line/70 bg-[#0d1424] p-1 shadow-soft">
          {loading ? <div className="flex items-center gap-2 px-3 py-3 text-sm font-semibold text-slate-500"><RefreshCw className="h-4 w-4 animate-spin" />Loading suggestions...</div> : visibleOptions.length ? visibleOptions.map((option) => <button key={option.appVersion} type="button" role="option" aria-selected={value === option.appVersion} onMouseDown={(event) => event.preventDefault()} onClick={() => { onChange(option.appVersion); setIsOpen(false); }} className={`focus-ring block w-full rounded-[7px] px-3 py-2 text-left transition-colors hover:bg-[#17223a] ${value === option.appVersion ? "bg-emerald/10 text-emerald" : "text-slate-400"}`}><span className="block text-sm font-bold text-slate-200">{option.appVersion}</span><span className="mt-1 block text-xs">{formatter.format(option.sampleCount)} samples</span></button>) : <div className="px-3 py-3 text-sm text-slate-500">No matching suggestions. You can still run this typed version.</div>}
        </div> : null}
      </div>
...
export function FunnelDateRangePicker({ startDate, endDate, onChange }: { startDate: string; endDate: string; onChange: (range: DateRange) => void }) {
  ...
  return <div className="relative"><span className={labelClass}>Date range</span><button ref={triggerRef} type="button" onClick={() => isOpen ? setIsOpen(false) : openPicker()} className={`${inputClass} flex items-center justify-between gap-3 text-left`}><span className="min-w-0 truncate text-sm">{startDate} to {endDate}</span><CalendarDays className="h-4 w-4 shrink-0 text-white" /></button>{isOpen && popoverPosition && typeof document !== "undefined" ? createPortal(<div className="fixed z-[100] overflow-hidden rounded-xl border border-line/70 bg-[#0d1424] shadow-soft" style={{ top: popoverPosition.top, left: popoverPosition.left, width: popoverPosition.width }}><div className="grid max-h-[520px] grid-cols-1 md:grid-cols-[160px_1fr]"><div className="border-b border-line/60 bg-[#0a111e] p-3 md:border-b-0 md:border-r"><div className="flex max-h-64 flex-col gap-1 overflow-y-auto pr-1">{presets.map((preset) => <button key={preset.label} type="button" onClick={() => { const range = preset.range(); setDraftStart(range.startDate); setDraftEnd(range.endDate); setVisibleMonth(startOfMonth(parseIsoDate(range.startDate))); onChange(range); setIsOpen(false); }} className="focus-ring rounded-[7px] px-3 py-2 text-left text-sm font-semibold text-slate-400 hover:bg-[#17223a] hover:text-slate-200">{preset.label}</button>)}</div></div><div className="p-4"><div className="mb-4 flex flex-wrap items-center gap-3 text-xs font-bold uppercase text-slate-500"><span>Start</span><span className="rounded-[7px] border border-line/70 bg-[#0a111e] px-3 py-2 font-mono text-slate-300">{draftStart || "Select date"}</span><span>End</span><span className="rounded-[7px] border border-line/70 bg-[#0a111e] px-3 py-2 font-mono text-slate-300">{draftEnd || "Select date"}</span><div className="ml-auto flex gap-2"><button type="button" onClick={() => setVisibleMonth((current) => addMonths(current, -1))} className="focus-ring flex h-9 w-9 items-center justify-center rounded-[7px] border border-line/70 bg-[#101a2c] text-slate-400 hover:bg-[#17223a] hover:text-slate-200" aria-label="Previous month"><ChevronLeft className="h-4 w-4" /></button><button type="button" onClick={() => setVisibleMonth((current) => addMonths(current, 1))} className="focus-ring flex h-9 w-9 items-center justify-center rounded-[7px] border border-line/70 bg-[#101a2c] text-slate-400 hover:bg-[#17223a] hover:text-slate-200" aria-label="Next month"><ChevronRight className="h-4 w-4" /></button></div></div><div className="grid gap-6 lg:grid-cols-2">{renderMonth(visibleMonth)}{renderMonth(addMonths(visibleMonth, 1))}</div><div className="mt-5 flex justify-end gap-2 border-t border-line pt-4"><button type="button" onClick={() => setIsOpen(false)} className="focus-ring h-10 rounded-[8px] border border-line/70 bg-[#121b2c] px-4 text-sm font-semibold text-slate-300 hover:bg-[#17223a]">Cancel</button><button type="button" disabled={!draftStart || !draftEnd} onClick={() => { if (!draftStart || !draftEnd) return; onChange({ startDate: draftStart, endDate: draftEnd }); setIsOpen(false); }} className="focus-ring h-10 rounded-md bg-cobalt px-4 text-sm font-semibold text-white hover:bg-cobalt/90 disabled:opacity-50">Apply</button></div></div></div></div>, document.body) : null}</div>;
}
