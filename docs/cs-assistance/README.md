...
## Focused dispute inputs (local update)

Dispute type is now required: **Items / coins not received** or **No ads not working**. Purchase date (UTC) and purchase amount in USD are optional. Amounts match `dollar_value` to cents; they are not converted from local currency. Ticket date still controls the 90-day inspection window. Dates after the ticket or outside the window are rejected with a correction prompt.

One exact date/amount match proceeds automatically. Multiple matches or no exact match show purchase candidates (closest dates first when a date was supplied), including timestamp, USD amount and transaction ID. CS must explicitly select a candidate. All purchases remain in the evidence context to detect overlapping benefits, but the recommendation only covers the selected purchase and dispute. Item complaints for VIP Pass do not use the no-ads expiry conclusion.

User IDs can change on reinstall. `COUNT(DISTINCT user_id)` is descriptive metadata, not an ownership or eligibility gate; the IDFV-scoped trace spans those IDs.

API: submit `{ idfv, dispute: "items" | "no_ads", purchaseDate?: "YYYY-MM-DD", amountUsd?: "14.99", asOf?: "YYYY-MM-DD" }`. A poll may return `selection_required`; resubmit the same token with `selectedId` to assess a candidate. Filters are encrypted into the user-bound token and cannot be changed on the polling request. Expired tokens require a new check.

## Response hierarchy

The bot response leads with recommendation (including inferred/observed basis), finding and CS next step. Purchase and dispute scope follow as secondary context. Evidence and check details are collapsed by default in the web preview. Important uncertainty stays in the finding. Copy response copies the concise main message. Full timestamps, transaction references, event evidence, coverage and limitations remain available in the supporting sections.
