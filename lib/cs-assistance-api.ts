...
export async function handleAssistance(request: Request, polling = false) {
  try {
    ...
    const result = polling
      ? await pollAssistance(z.object({ token: z.string().min(1).max(8192) }).parse(body).token, user.id, z.object({ selectedId: z.string().max(300).optional() }).parse(body).selectedId)
