...
const ticketSchema = z.object({ owner: z.string(), queryId: z.string(), expiresAt: z.number(), createdAt: z.number() });
function key() {
...
export async function startAssistance(body: unknown, owner: string, now = Date.now()) {
  ...
  return { status: "pending" as const, token: sealTicket({ owner, queryId, expiresAt, createdAt: now }), expiresAt, window };
}
export async function pollAssistance(token: string, owner: string) {
  ...
  const assessment = assess(response.trace);
  ...
  return { status: "completed" as const, assessment, botSections, checkedAt: new Date().toISOString() };
}
