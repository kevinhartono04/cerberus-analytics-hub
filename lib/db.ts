...
async function ensureGameplayAlertTables() {
  if (!gameplayAlertTablesReady) {
    ...
    gameplayAlertTablesReady = sql.begin(async (transaction) => {
      ...
      await transaction`ALTER TABLE gameplay_alert_states ADD COLUMN IF NOT EXISTS level_id TEXT`;
...
function ensureSqliteGameplayAlertTables() {
  sqliteExec(`
    ...
    CREATE TABLE IF NOT EXISTS gameplay_alert_states (
      ...
      level INTEGER NOT NULL, level_id TEXT, layout_bank_id TEXT, layout_hash TEXT, difficulty_tier TEXT NOT NULL, status TEXT NOT NULL, first_seen_at TEXT NOT NULL,
  ...
  if (!sqliteColumnExists("gameplay_alert_settings", "exclude_test_countries")) sqliteExec("ALTER TABLE gameplay_alert_settings ADD COLUMN exclude_test_countries INTEGER NOT NULL DEFAULT 1");
  ...
  if (!sqliteColumnExists("gameplay_alert_states", "level_id")) sqliteExec("ALTER TABLE gameplay_alert_states ADD COLUMN level_id TEXT");
...
function rowToGameplayAlertSettings(row: Record<string, unknown>): GameplayAlertSettingsRecord {
  ...
  return {
    ...
    excludeTestCountries: row.exclude_test_countries === undefined || row.exclude_test_countries === null
      ? true
      : row.exclude_test_countries === true || row.exclude_test_countries === 1 || row.exclude_test_countries === "1" || row.exclude_test_countries === "true",
...
function rowToGameplayAlertState(row: Record<string, unknown>): GameplayAlertStateRecord {
  return {
    ...
    level: Number(row.level), ...(asString(row.level_id) ? { levelId: asString(row.level_id) } : {}), ...(asString(row.layout_bank_id) ? { layoutBankId: asString(row.layout_bank_id) } : {}), ...(asString(row.layout_hash) ? { layoutHash: asString(row.layout_hash) } : {}), difficultyTier: asString(row.difficulty_tier) === "hard" ? "hard" : "normal",
...
export async function saveGameplayAlertStateRecords(records: GameplayAlertStateRecord[]) {
  ...
  if (shouldUseLocalSqlite()) {
    ensureSqliteGameplayAlertTables();
    for (const record of records) {
      sqliteExec(`INSERT INTO gameplay_alert_states (alert_key, alert_kind, app_name, platform, app_version, level, level_id, layout_bank_id, layout_hash, difficulty_tier, status, first_seen_at, last_seen_at, resolved_at, superseded_at, last_fail_rate, last_reached_players, threshold, slack_open_delivered_at, slack_pending_delivered_at, slack_resolved_delivered_at) VALUES (${sqliteLiteral(record.alertKey)}, ${sqliteLiteral(record.alertKind)}, ${sqliteLiteral(record.appName)}, ${sqliteLiteral(record.platform)}, ${sqliteLiteral(record.appVersion)}, ${record.level}, ${record.levelId ? sqliteLiteral(record.levelId) : "NULL"}, ${record.layoutBankId ? sqliteLiteral(record.layoutBankId) : "NULL"}, ${record.layoutHash ? sqliteLiteral(record.layoutHash) : "NULL"}, ${sqliteLiteral(record.difficultyTier)}, ${sqliteLiteral(record.status)}, ${sqliteLiteral(record.firstSeenAt)}, ${sqliteLiteral(record.lastSeenAt)}, ${record.resolvedAt ? sqliteLiteral(record.resolvedAt) : "NULL"}, ${record.supersededAt ? sqliteLiteral(record.supersededAt) : "NULL"}, ${record.lastFailRate}, ${record.lastReachedPlayers}, ${record.threshold}, ${record.slackOpenDeliveredAt ? sqliteLiteral(record.slackOpenDeliveredAt) : "NULL"}, ${record.slackPendingDeliveredAt ? sqliteLiteral(record.slackPendingDeliveredAt) : "NULL"}, ${record.slackResolvedDeliveredAt ? sqliteLiteral(record.slackResolvedDeliveredAt) : "NULL"}) ON CONFLICT(alert_key) DO UPDATE SET alert_kind = excluded.alert_kind, level_id = excluded.level_id, layout_bank_id = excluded.layout_bank_id, layout_hash = excluded.layout_hash, status = excluded.status, last_seen_at = excluded.last_seen_at, resolved_at = excluded.resolved_at, superseded_at = excluded.superseded_at, last_fail_rate = excluded.last_fail_rate, last_reached_players = excluded.last_reached_players, threshold = excluded.threshold, slack_open_delivered_at = excluded.slack_open_delivered_at, slack_pending_delivered_at = excluded.slack_pending_delivered_at, slack_resolved_delivered_at = excluded.slack_resolved_delivered_at`);
    }
    return;
  }
  const sql = await ensureGameplayAlertTables();
  for (const record of records) {
    await sql`INSERT INTO gameplay_alert_states (alert_key, alert_kind, app_name, platform, app_version, level, level_id, layout_bank_id, layout_hash, difficulty_tier, status, first_seen_at, last_seen_at, resolved_at, superseded_at, last_fail_rate, last_reached_players, threshold, slack_open_delivered_at, slack_pending_delivered_at, slack_resolved_delivered_at) VALUES (${record.alertKey}, ${record.alertKind}, ${record.appName}, ${record.platform}, ${record.appVersion}, ${record.level}, ${record.levelId ?? null}, ${record.layoutBankId ?? null}, ${record.layoutHash ?? null}, ${record.difficultyTier}, ${record.status}, ${record.firstSeenAt}, ${record.lastSeenAt}, ${record.resolvedAt ?? null}, ${record.supersededAt ?? null}, ${record.lastFailRate}, ${record.lastReachedPlayers}, ${record.threshold}, ${record.slackOpenDeliveredAt ?? null}, ${record.slackPendingDeliveredAt ?? null}, ${record.slackResolvedDeliveredAt ?? null}) ON CONFLICT(alert_key) DO UPDATE SET alert_kind = excluded.alert_kind, level_id = excluded.level_id, layout_bank_id = excluded.layout_bank_id, layout_hash = excluded.layout_hash, status = excluded.status, last_seen_at = excluded.last_seen_at, resolved_at = excluded.resolved_at, superseded_at = excluded.superseded_at, last_fail_rate = excluded.last_fail_rate, last_reached_players = excluded.last_reached_players, threshold = excluded.threshold, slack_open_delivered_at = excluded.slack_open_delivered_at, slack_pending_delivered_at = excluded.slack_pending_delivered_at, slack_resolved_delivered_at = excluded.slack_resolved_delivered_at`;
  }
}
