...
export function assessObserved(trace: Trace, catalog: Record<string, CatalogProduct> = productCatalog(), dispute?: "items" | "no_ads"): Assessment {
  ...
  const findings = o.purchases.map((p): Finding => {
    ...
    // Reinstallation changes user_id; the IDFV-scoped history intentionally spans these IDs.
    ...
    if (dispute === "items" || (!dispute && product.noAds === "none")) {
    ...
    if (product.noAds === "none") return review("The catalog lists no no-ads benefit for this product. Confirm the disputed product and offer terms.");
