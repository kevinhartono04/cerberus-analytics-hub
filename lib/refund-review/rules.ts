...
const review = "Review the purchase and missing or conflicting evidence before deciding a refund.";
export function assess(input: Trace, products: Record<string, Product> = demoProducts): Assessment {
  const trace = traceSchema.parse(input);
  if (trace.observed) return assessObserved(trace);
  ...
  const findings = trace.purchases.map((p): Finding => {
    ...
    const set = (verdict: Finding["verdict"], reason: string, ids: string[] = []) => { f.verdict = verdict; f.reason = reason; f.evidence.push(...ids); f.evidence = [...new Set(f.evidence)]; f.nextStep = verdict === "Recommend eligible" ? "Verify this is the disputed purchase and review a refund." : verdict === "Recommend not eligible" ? "Explain the observed delivery or benefit terms; confirm this is the disputed purchase." : review; return f; };
    ...
    if (product.items && product.noAds) return set("Needs review", "Bundled benefits require a dedicated product rule.");
    if (product.items) {
      ...
      if (delivered) return set("Recommend not eligible", "Expected items and quantities were granted for this purchase.", grants.map(g => g.id));
    ...
    if (expiredAds.length) {
      ...
      return set("Recommend not eligible", "Interstitial ads resumed after the temporary no-ads benefit expired.", expiredAds.map(a => a.id));
    }
    const rewarded = ads.filter(a => a.kind === "rewarded");
    if (rewarded.length) return set("Recommend not eligible", "Only rewarded ads were observed; this pack blocks interstitial ads, not rewarded ads.", rewarded.map(a => a.id));
