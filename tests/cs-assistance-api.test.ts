...
const request = (body: unknown, origin?: string) => new Request("http://localhost/api/cs-assistance", { method: "POST", headers: origin ? { origin } : {}, body: JSON.stringify({ dispute: "no_ads", purchaseDate: "2026-08-19", amountUsd: "14.99", ...(body as object) }) });
...
describe("CS Assistance authenticated live flow", () => {
 ...
 it("accepts matching browser Host behind a local proxy", async () => { const r = new Request("http://localhost/api/cs-assistance", { method: "POST", headers: { host: "127.0.0.1:3101", origin: "http://127.0.0.1:3101" }, body: JSON.stringify({ idfv, dispute: "no_ads" }) }); expect((await start(r)).status).toBe(200); });
...
it("requires dispute, validates purchase date and amount, and preserves filters in the token", async()=>{
 expect((await start(request({idfv,dispute:undefined}))).status).toBe(400);
 expect((await start(request({idfv,amountUsd:"14.999"}))).status).toBe(400);
 expect((await start(request({idfv,purchaseDate:"2026-09-09",asOf:"2026-09-08"}))).status).toBe(400);
 const started=await (await start(request({idfv,purchaseDate:"",amountUsd:""}))).json();
 expect(openTicket(started.token,"internal-1").review).toEqual({dispute:"no_ads",purchaseDate:"",amountUsd:""});
 const observed=observedSchema.parse(JSON.parse(fs.readFileSync("tests/fixtures/refund-review/sample-facts.json","utf8")));mocks.poll.mockResolvedValue({status:"complete",trace:observedToTrace(observed)});
 const pending=await (await poll(request({token:started.token}))).json();expect(pending.status).toBe("selection_required");
 const result=await (await poll(request({token:started.token,selectedId:observed.purchases[0].id}))).json();expect(result.assessment.findings).toHaveLength(1);
});
