...
import { checkDetailsPage, render } from "@/lib/refund-review/slack";
...
describe("real catalog and Sept 8 case", () => {
  ...
  it("does not treat inactivity, immediate ads, or overlapping no-ads purchases as expiry", () => {
    for (const kind of ["inactive", "immediate", "overlap", "unknown", "incomplete"] as const) {
      const o = sample();
  ...
  it("labels inference and formats live output without a demo label or false freshness promise", () => {
    ...
    expect(rendered).toContain("(inferred)"); expect(rendered).toContain("$14.99"); expect(rendered).not.toContain("Last observed activity"); expect(JSON.stringify(checkDetailsPage(job))).toContain("Last observed activity");
...
describe("input, signed resource movements and query completeness", () => {
  it("keeps IDFV-only input and accepts a valid ticket-date option", () => {
    expect(parseIdfv("<@BOT> 00000000000000000000000000000001 dispute:no_ads asof:2026-09-08")).toEqual({ idfv: "00000000000000000000000000000001", asOf: "2026-09-08", review: {dispute: "no_ads", purchaseDate: "", amountUsd: ""} });
